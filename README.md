...
└── scraper/
    ...
    ├── jobplanet_web.py      # 공고: 잡플래닛(curl_cffi로 Cloudflare 우회 + 내부 JSON API)
...
- **닉네임 로그인(우측 상단)**: 비밀번호 없이 닉네임만 입력 → 그 닉네임의 찜/숨김 내역을 불러옴. 서버(`serve.py` 또는 배포한 Worker) 연결 시 **기기 간 동기화**. 미연결(정적 단독)이면 닉네임별 localStorage로 기기 한정 저장. 백엔드 배포는 [`server/README.md`](server/README.md)
...
| **잡플래닛(공고)** | curl_cffi + 내부 JSON API | Cloudflare WAF로 일반 requests는 403 → curl_cffi(chrome TLS 모사)로 우회 후 `/api/v3/job/postings?query=` 호출. 미설치 시 자동 건너뜀. csn 없어 회사정보는 별도 연결 |
